...
class QueryParser():
    ...
    def parse_query(self):
        ...
        main_search_result = re.search(
            main_pattern, self.query_str, re.IGNORECASE
        )
        ...
        tables = [
            table for table in main_tables_with_cte_alias
        ...
        for table in tables:
            line = [
                i for i, line in enumerate(self.query_str.splitlines())
                if table in line
            ][0]
            ref_tables.append({
                'table_name': table,
